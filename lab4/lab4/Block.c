 ...
 * Description:
 * 		Calculate the hash value of a block, if block or data is NULL, return -1
 */
long hash(Block B) {
	long result = -1;
	int index;
	if (B != NULL) {
		...
		if ( B -> data == NULL ) {
			fprintf(stderr, "The data of the Block passed was NULL, please pass a block with valid data.\n");
			return -1;
		} else {
...
void printBlock(FILE* out, Block B) {
	if ( B == NULL ) {
		fprintf(stderr, "The Block is NULL, please pass a valid Block and try again.\n");
	} else {
		fprintf(out, "%d:%s\n", B -> id, B -> data);
	}
}
